...
fn build_state_with_previous(
    ...
    let hosts = match mode {
        Mode::Mock => crate::fixtures::fixture_hosts_for_config(config)
            .into_iter()
            .enumerate()
            .map(|(idx, host)| {
            ...
            .collect(),
        Mode::Live => collect_live_hosts(generated_at, previous, config),
    };
...
fn collect_live_hosts(
    ...
    let collected = collect_enabled_servers(config);
...
) -> Vec<(crate::config::ServerConfig, HostMetrics)> {
    let servers = enabled_servers(config).cloned().collect::<Vec<_>>();
        ...
        let handles = servers
            ...
            .map(|server| {
                scope.spawn(move || match crate::collectors::collect_host(&server) {
                    Ok(metrics) => Some((server, metrics)),
...
fn run_loop(
    ...
    let refresh_interval = Duration::from_millis(config.refresh_interval_ms);
    let dashboard = build_state_with_previous(mode, None, refresh_interval, &config)?;
    let mut state = TuiState::new(dashboard, refresh_interval);
...
fn start_dashboard_refresh(
    ...
    thread::spawn(move || {
        let result =
            build_state_with_previous(mode, Some(&thread_previous), refresh_interval, &config);
...
fn poll_dashboard_refresh(
    ...
    match rx.try_recv() {
        Ok(refresh) => {
            let mut dashboard = refresh.result?;
            dashboard.refresh_interval_ms = state.refresh_interval.as_millis() as u64;
            state.replace_dashboard_after_refresh(dashboard);
            *refresh_rx = None;
        }
...
"#,
    ...
    fn optional_unavailable_servers_are_hidden_from_live_collection() {
        ...
        assert!(
            collect_enabled_servers(&optional_config).is_empty(),
        ...
        assert_eq!(required_hosts.len(), 1);
        assert!(matches!(required_hosts[0].1.status, HostStatus::Offline));
    }
