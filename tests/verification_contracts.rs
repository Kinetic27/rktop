...
const EXAMPLE_CONFIG: &str = include_str!("../config/rktop.example.toml");
...
fn default_config_is_empty_and_first_run_does_not_embed_private_servers() {
    ...
    assert_contains(
        APP_RS,
        "OptionalFailed(server.id)",
        "optional SSH failures should hide that host instead of rendering an unavailable card",
    );
    assert_contains(
        APP_RS,
        "OPTIONAL_REFRESH_RETRY_AFTER",
        "optional SSH failures should not block every short-interval refresh cycle",
    );
}
...
fn first_run_config_flow_is_wired_into_cli() {
    ...
    assert_contains(
        CONFIG_RS,
        "XDG_CONFIG_HOME",
        "user config path should follow XDG_CONFIG_HOME when present",
    );
    assert_contains(
        CONFIG_RS,
        "APP_CONFIG_DIR: &str = \"rktop\"",
        "default app config directory",
    );
    assert_contains(
        CONFIG_RS,
        "RKTOP_CONFIG",
        "runtime config lookup should support an explicit environment override",
    );
    assert_contains(
        CONFIG_RS,
        "PathBuf::from(\"/etc\")",
        "runtime config lookup should support a Linux system-wide fallback",
    );
    assert_contains(
        README_MD,
        "/etc/rktop/config.toml",
        "README should document the system-wide Linux config fallback",
    );
...
#[test]
fn github_release_deb_packaging_is_wired() {
    assert_contains(
        BUILD_DEB_SH,
        "Package: $pkg_name",
        "Debian control package name generation",
    );
    assert_contains(
        BUILD_DEB_SH,
    ...
    assert_contains(
        RELEASE_YML,
        "tags:",
        "release workflow should run for version tags",
    );
    assert_contains(
        RELEASE_YML,
        "scripts/build-deb.sh",
        "release workflow should build the Debian package",
    );
    assert_contains(
        RELEASE_YML,
        "gh release create",
        "release workflow should upload package artifacts",
    );
    assert_contains(
        README_MD,
        "scripts/build-deb.sh",
...
#[test]
fn windows_ssh_runner_release_is_documented_and_wired() {
    assert_contains(
        CI_YML,
        "windows-latest",
        "CI should check that the native Windows runner compiles",
    );
    assert_contains(
        RELEASE_YML,
        "build windows package",
        "release workflow should build a native Windows package",
    );
    assert_contains(
        RELEASE_YML,
        "rktop.exe",
        "Windows release should expose the preferred rktop.exe command",
    );
    assert_contains(
        RELEASE_YML,
        "Compress-Archive",
    ...
    assert_contains(
        APP_RS,
        "USERPROFILE",
        "Windows SSH config discovery should check the user profile .ssh directory",
    );
    assert_contains(
        LOCAL_RS,
        "local collection is only supported on Linux",
        "non-Linux native runners should fail local collection explicitly",
    );
    assert_contains(
        README_MD,
    ...
    assert_contains(
        README_MD,
        "native `rktop.exe` can monitor Linux SSH hosts",
        "README should scope Windows support to SSH monitoring",
    );
}
