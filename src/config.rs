...
pub const APP_CONFIG_DIR: &str = "server-tui-monitor";
...
pub fn load_config(explicit_path: Option<&Path>) -> Result<LoadedConfig> {
    match explicit_path {
        ...
        None => match default_config_path() {
            Some(path) if path.exists() => load_config_file(&path).map(|config| LoadedConfig {
                config,
                origin: ConfigOrigin::File(path),
            }),
            default_path => Ok(LoadedConfig {
                config: default_config(),
                origin: ConfigOrigin::BuiltIn { default_path },
            }),
        },
    }
}
...
pub fn canonical_config_toml(config: &AppConfig) -> String {
    let mut output = String::from(
        "# Server TUI Monitor config\n\
# Default path: ~/.config/server-tui-monitor/config.toml\n\
#\n\
# source can be:\n\
